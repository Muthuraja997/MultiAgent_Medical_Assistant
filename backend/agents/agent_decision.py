...
class AgentConfig:
    ...
    7. PSYCHOLOGY_AGENT - For mental health support, counseling, stress management, anxiety, depression, and emotional wellbeing. Uses MentalBERT AI model for analysis.
    ...
    _image_analyzer = None  # Lazy initialization
    
    @classmethod
    def get_image_analyzer(cls):
        """Lazy getter for image analyzer"""
...
def create_agent_graph():
    ...
    def route_to_agent(state: AgentState) -> Dict:
        ...
        try:
            ...
            print(f"[WARNING] Gemini API quota exceeded: {str(e)}")
            print("[INFO] Falling back to CONVERSATION_AGENT with quota warning")
            
            # Fallback decision when quota is exceeded
            decision = {
        ...
        **You:** "I'm not a doctor, but headaches and fever can have various causes, from infections to dehydration. If your symptoms persist, you should see a medical professional."
        ...
        try:
            ...
            if "429" in error_msg or "quota" in error_msg.lower() or "ResourceExhausted" in error_msg:
                response = AIMessage(content="""I apologize, but I've temporarily reached my API request limit. 

🔄 **What you can do:**
- Wait about 1 minute and try again (free tier resets every minute)
- Your question was: "{}"

💡 **Alternative options:**
- Upload medical images for analysis (doesn't use quota)
- Ask specific medical questions when the limit resets

The free tier has a limit of 20 requests per minute. Please try again shortly!""".format(input_text))
            else:
                response = AIMessage(content=f"I encountered an error processing your request: {error_msg}. Please try again.")
    ...
    def run_rag_agent(state: AgentState) -> AgentState:
        ...
        # Lazy import and initialization of RAG agent
        from agents.rag_agent import MedicalRAG
    ...
    def run_web_search_processor_agent(state: AgentState) -> AgentState:
        ...
        # Lazy import of WebSearchProcessorAgent
        from agents.web_search_processor_agent import WebSearchProcessorAgent
    ...
    def run_psychology_agent(state: AgentState) -> AgentState:
        """Handle mental health support and psychological counseling."""
        
        print(f"Selected agent: PSYCHOLOGY_AGENT")
        ...
        try:
            ...
            # Add context about the analysis (for logging/monitoring)
            primary_concern = result.get("primary_concern", "general_mental_health")
            concern_confidence = result.get("concern_confidence", 0.0)
            is_crisis = result.get("crisis_assessment", {}).get("is_crisis", False)
            
            # Log important information
            print(f"Mental Health Analysis - Primary Concern: {primary_concern} (confidence: {concern_confidence:.2f})")
            if is_crisis:
                print("⚠️ CRISIS DETECTED - Response includes emergency resources")
            ...
            return {
                **state,
                "output": response,
                "needs_human_validation": False,  # Psychology support doesn't need validation, but crisis cases have emergency info
                "agent_name": "PSYCHOLOGY_AGENT"
            }
            
        except Exception as e:
            print(f"Error in psychology agent: {e}")
            # Fallback response
            fallback_message = (
                "I'm here to support you with your mental health concerns. While I can provide general guidance, "
                "I strongly encourage you to speak with a licensed mental health professional for personalized care.\n\n"
                "**Immediate Resources:**\n"
    ...
    workflow.add_node("PSYCHOLOGY_AGENT", run_psychology_agent)
    ...
    workflow.add_edge("PSYCHOLOGY_AGENT", "check_validation")
